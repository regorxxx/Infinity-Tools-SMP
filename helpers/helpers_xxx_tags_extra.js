...
function updateTrackSimilarTags({ data, iNum = 10, tagName = globTags.lbSimilarArtist, windowName = window.FullPanelName, bPopup = true } = {}) {
...
function getSimilarDataFromFile(file, newData = null, iNum = Infinity) {
	const data = _jsonParseFileCheck(file, 'Tags json', window.FullPanelName, utf8);
	if (data) {
		if (newData) {
