...
function checkTags({
	...
	if (bUseDic) {
		if (dictionary.dictionary !== properties.dictName[1]) {
			...
			fb.ShowPopupMessage('Dictionary path not found:\n' + dictSettings.getDicPath() + '\n' + dictSettings.getAffPath(), window.FullPanelName); return;
		}
	}
	// Constants
...
function checkTagsReport(tagsToCheck, tagsArt, countArrayFiltered, keySplit, alternativesMap, popupTitle, properties, tagValuesExcluded, missingArt) {
	...
	if (properties['bAskForConfigTags'][1]) {
		...
		let answer = WshShell.Popup('Do you want to add new tags for exclusion in future reports?', 0, window.FullPanelName, popup.question + popup.yes_no);
		if (answer === popup.yes) {
			let inputTags = utils.InputBox(window.ID, 'Tag pair(s) to exclude from future reports\n(Values known to be right)\n Pairs \'tagName,value\' separated by \';\' :', window.FullPanelName, currentTags);
...
function loadTagsExcluded(path) { // filter holes and remove duplicates
	...
	let obj = bFromFile
		? _jsonParseFileCheck(path, 'Exclusion list json', window.FullPanelName, utf8) || {}
		: {};
