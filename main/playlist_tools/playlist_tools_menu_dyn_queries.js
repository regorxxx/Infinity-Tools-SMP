			...
			if (_isFile(scriptPath)) {
				...
				if (!Object.hasOwn(menusEnabled, name) || menusEnabled[name]) {
					...
					{	// Dynamic menu
						...
						// Menus
						menu.newEntry({ menuName: subMenuName, entryText: 'Based on queries evaluated with sel:', func: null, flags: MF_GRAYED });
							...
							entryText: 'Dynamic Queries (cond)', condFunc: () => {
								...
								queryFilter.forEach((queryObj) => {
									...
									if (menu.isSeparator(queryObj)) {
										...
										// Entries
										menu.newEntry({
											menuName: subMenuName, entryText: queryName, func: () => {
								...
								{ // Static menu: user configurable
									menu.newEntry({
										menuName: subMenuName, entryText: 'By... (query)', func: () => {
											let input = '';
				...
				} else { menuDisabled.push({ menuName: name, subMenuFrom: libSearchMenu, index: menu.getMenus().filter((entry) => menuAltAllowed.has(entry.subMenuFrom)).length + disabledCount++, bIsMenu: true }); } // NOSONAR
			}
		}
	}
}