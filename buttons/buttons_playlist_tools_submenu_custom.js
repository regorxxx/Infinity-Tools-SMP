...
/* global MK_SHIFT:readable, MK_CONTROL:readable */
...
addButton({
	'Playlist Tools SubMenu (CUSTOM)': new ThemedButton({
		...
		variables: {
			setIcon: function (parent, name) {
				...
				switch (name.toLowerCase()) { // NOSONAR
					...
					case 'select (# tracks)':
					case 'select (by time)': icon = chars.checkSquareAlt; break;
					case 'expand (disjoint)':
					case 'expand (contiguous)':
					case 'expand (next)': icon = chars.expand; break;
				...
				if (icon !== this.icon) {
					this.icon = icon;
					this.iconWidth = _textWidth(this.icon, this.gFontIcon);
				}
			}
		},
		onInit: function () {
			this.setIcon(this.text);
		}
	}),
});
