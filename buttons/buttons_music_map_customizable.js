				...
				switch (name) { // NOSONAR
					case sbd.name + ': share configuration': {
						if (info) {
							...
							const answer = WshShell.Popup('Apply current configuration to highlighted button?\nCheck buttons bar.', 0, window.FullPanelName + ': ' + sbd.name, popup.question + popup.yes_no);
							if (answer === popup.yes) {
