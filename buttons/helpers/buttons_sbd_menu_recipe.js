...
function createRecipeMenu(parent) {
	...
	const newRecipe = (name = '') => {
		...
		if (WshShell.Popup('Include tag remapping?', 0, window.FullPanelName, popup.question + popup.yes_no) === popup.no) {
			for (let key in recipe.tags) {
				delete recipe.tags[key].tf;
				delete recipe.tags[key].type;
			}
		}
		if (WshShell.Popup('Also add additional variables?\n' + [...recipePropertiesAllowedKeys].joinEvery(', ', 4), 0, window.FullPanelName, popup.question + popup.yes_no) === popup.yes) {
			recipe.properties = {};
