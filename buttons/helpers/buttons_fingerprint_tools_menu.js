		...
		const config = menu.newMenu('Configuration');
		{
			...
			menu.newEntry({
				menuName, entryText: 'Read directly from files' + (bChromaprint ? '' : '\t(not installed)'), func: () => {
					...
					overwriteProperties(ppt);
					if (ppt.bReadFiles[1]) {
						fb.ShowPopupMessage('Instead of caching the fingerprint tag within foobar, tweaking the \'LargeFieldsConfig.txt\' file, fingerprint tag may be directly read from the files (not for files containing subsongs, like .iso or .cue files).\n\nCreating the database will be slower, since it will have to process the entire library reading from files, but it will offer similar performance afterwards bypassing some RAM limitations when caching the tag.', window.FullPanelName);
					}
				}, flags: flagsChroma
			});
			...
			menu.newEntry({
				menuName, entryText: 'Score threshold' + '\t' + _b(ppt.thresholdC[1]), func: () => {
					const input = Input.number('int positive', ppt.thresholdC[1], 'Enter number:\n(≥0 and ≤100)', 'Fingerprint Tools', ppt.thresholdC[3], [(input) => input >= 0 && input <= 100]);
		...
		}
		{
			...
			menu.newEntry({
				menuName, entryText: 'Score threshold' + '\t' + _b(ppt.thresholdF[1]), func: () => {
					const input = Input.number('int positive', ppt.thresholdF[1], 'Enter number:\n(≥0 and ≤100)', 'Fingerprint Tools', ppt.thresholdF[3], [(input) => input >= 0 && input <= 100]);
					if (input === null) { return; }
		...
		{	// Config max Selection
			menu.newEntry({
				menuName: config, entryText: 'Max selection allowed' + '\t' + _b(maxSel), func: () => {
					const input = Input.number('int positive', maxSel, 'Enter number:\n(integer number ≥0)', 'Fingerprint Tools', ppt.iMaxSelection[3], [(input) => input >= 0]);
					if (input === null) { return; }
