...
function createConfigMenu(parent) {
		...
		{	// New tag
			menu.newEntry({
				menuName, entryText: 'New tag...', func: () => {
					...
					if (name === null) { return; }
					if (WshShell.Popup('Is multi-valued?\n\nTag may make use of multiple or single values. For ex. GENRE usually have more than one value, while DATE is meant to store a single value.\nSingle-valued configured tags will skip any value past the first one.\nMulti-valued tags can only be of \'string\' type.', 0, window.FullPanelName, popup.question + popup.yes_no) === popup.yes) { nTag.type.push('multiple', 'string'); }
					else { nTag.type.push('single'); }
					if (nTag.type.includes('single')) {
						if (WshShell.Popup('Are tag values strings?\n\nTag values may be considered as strings or numbers. For ex. GENRE, TITLE, etc. are strings, while DATE or BPM are numbers.', 0, window.FullPanelName, popup.question + popup.yes_no) === popup.yes) { nTag.type.push('string'); }
						else { nTag.type.push('number'); }
						if (nTag.type.includes('string')) {
							if (WshShell.Popup('Is a genre/style-like tag?\n\nClicking yes will use the tag for GRAPH purposes, along the default GENRE and STYLE tags.', 0, window.FullPanelName, popup.question + popup.yes_no) === popup.yes) { nTag.type.push('graph'); }
						} else if (nTag.type.includes('number')) {
							if (WshShell.Popup('Uses absolute range?\n\nSince tag values are numbers, comparison is done within a range. Absolute range will make the configurable range to be used as an absolute value, i.e. 30 equals to +-30. For ex. for dates.', 0, window.FullPanelName, popup.question + popup.yes_no) === popup.yes) { nTag.type.push('absRange'); nTag.range = 0; }
							else if (WshShell.Popup('Uses percent range?\n\nSince tag values are numbers, comparison is done within a range. Percent range will make the configurable range to be used as a percent value, i.e. 30 equals to +-30% of the original value. For ex. for BPM.', 0, window.FullPanelName, popup.question + popup.yes_no) === popup.yes) { nTag.type.push('percentRange'); nTag.range = 0; }
						}
					} else {
	...
	if (!bLiteMode) {	// Special playlists:
		...
		}
		{
			...
			menu.newEntry({
				menuName, entryText, func: () => {
					const input = Input.number('int positive', properties[key][1], 'Enter number:\n(integer number ≥2 and ≤100)', sbd.name + ': ' + entryText.replace(/\t.*/, ''), properties[key][3], [(input) => input >= 2 && input <= 100]);
					if (input === null) { return; }
					properties[key][1] = input;
					overwriteProperties(properties); // Updates panel
	...
	menu.newSeparator();
	{
		...
		menu.newEntry({
			menuName: subMenuName, entryText: 'Rename button...', func: () => {
				let input = '';
				try { input = utils.InputBox(window.ID, 'Enter button name. Then configure according to your liking using the menus or the properties panel (look for \'' + parent.prefix + '...\').', window.FullPanelName + ': ' + sbd.name + ' Customizable Button', properties.customName[1], true); }
				catch (e) { return; } // eslint-disable-line no-unused-vars
