...
/* global MK_SHIFT:readable, VK_SHIFT:readable, globQuery:readable */
...
addButton({
	'Top Tracks from Date': new ThemedButton({
		coordinates: { x: 0, y: 0, w: _textWidth('Top ' + newButtonsProperties.playlistLength[1] + ' Tracks ' + (newButtonsProperties.year[1] || (new Date().getFullYear() - 1)), buttonsBar.config.font.text) + buttonsBar.config.buttonMargin, h: _scale(16, false) },
		...
		func: function (mask) {
			if (mask === MK_SHIFT) {
				...
				topTracksFromDate({ playlistLength: Number(this.buttonsProperties.playlistLength[1]), forcedQuery: this.buttonsProperties.forcedQuery[1], year: this.buttonsProperties.year[1] || (new Date().getFullYear() - 1), bProfile: typeof menu_panelProperties === 'undefined' ? false : menu_panelProperties.bProfile[1] });
			}
		},
