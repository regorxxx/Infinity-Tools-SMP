		...
		'Tagger': new ThemedButton({
			...
			func: function (mask) {
				...
				if (mask === MK_CONTROL) {
					...
					menu.newSeparator();
					menu.newEntry({ entryText: 'Open help...', func: () => showButtonReadme('buttons_tags_batch_tagger.js') });
			...
			description: function () {
				...
				info = _textLinesWrap(info, _gdiFont(font.name, font.size), 400).filter(isString).join('\n');
				// Modifiers
