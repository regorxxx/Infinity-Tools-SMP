			...
			func: function (mask) {
				...
				if (mask === MK_SHIFT || !list.length) {
					...
					_createSubMenuEditEntries(menu, void (0), {
						...
						input: () => {
							const entry = {
								command: Input.string('string', '', 'Enter complete menu name:\nEx: Library/Playback Statistics/Monitor playing tracks', window.FullPanelName + ': Main Menu Shortcut', 'Library/Playback Statistics/Monitor playing tracks', void (0), true),
								timeOut: Input.number('int positive', 0, 'Time (ms) to wait before running command:', window.FullPanelName + ': Main Menu Shortcut', 10) || 0,
							};
					...
					menu.newEntry({
						entryText: 'Rename button...', func: () => {
							const input = Input.string('string', this.buttonsProperties.customName[1], 'Enter button name:', window.FullPanelName + ': Main Menu Shortcut', this.buttonsProperties.customName[3], void (0), false);
					...
					menu.newEntry({
						entryText: 'Configure icon...', func: () => {
							const input = Input.string('unicode', this.buttonsProperties.icon[1], 'Enter button\'s icon: (unicode - fxxx)\n\nLook for values at:\ngithub.com/regorxxx/foobar2000-assets/blob/main/Fonts/FA_4_7_0.pdf', window.FullPanelName + ': Main Menu Shortcut', this.buttonsProperties.icon[3], void (0), false);
							if (input === null) { return; }
